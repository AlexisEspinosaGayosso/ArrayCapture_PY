...
if arrayType=="Inline":
    ...
    lowLeft=np.array([-LOneCyl/2.0,  -LOneCyl/2.0,  -ZOneCyl], dtype=np.float64)
    topRight=np.array([LOneCyl/2.0,  LOneCyl/2.0,  ZOneCyl], dtype=np.float64)
    cylCentresBase=np.array([[0, 0]], dtype=np.float64)
    cylNamesBase=list(["cylinder" ])
elif arrayType=="OneRow":
    lowLeft=np.array([-LOneCyl/2.0,  -LOneCyl/2.0,  -ZOneCyl], dtype=np.float64)
    topRight=np.array([LOneCyl/2.0,  LOneCyl/2.0,  ZOneCyl], dtype=np.float64)
    cylCentresBase=np.array([[0, 0]], dtype=np.float64)
    cylNamesBase=list(["cylinder" ])
...
print("Initial shape of the centres array:", centresClean.shape)
...
print("Updated shape of the centres array:", centresClean.shape)
print("The internally calculated centres are:")
print(centresClean)
...
####for ii in range(0,NCylinders):
    ...
    if os.path.isfile(outputName):
        ...
        #End 1st try
        forma=MAux.shape
        print("forma=", forma,  ", len(forma)=",  len(forma)) 
        if (len(forma) >1):
            nLines=forma[0]
        else:
            nLines=1
        ...
        if nLines>1:
            ...
            os.remove(outputName) #Deleting the file if we only have 1 lines or less. No Header.
        #End if nLines
    ...
    if (existingLast<arrayTimeLast):    
        ...
        for jj in range(startIndexTimes,Ntimes):        
            ...
            if jTimeN > existingLast: #But we really start with jTimeN > existingLast, not exactly at existingLast
                dataName=os.path.join(volumetricDir, jTimeC, iName)
                print("Reading data file:", dataName)
                dataArray=np.genfromtxt(dataName, delimiter=None) #, usecols=(0, 1)) 
                dataArrayLocal=dataArray[rowIni:rowEnd, :]
                            
                #Reading the coordinates and values, and converting to local polar coordinates
                xLocal=dataArrayLocal[:, 0]-centresClean[ii, 0]
                yLocal=dataArrayLocal[:, 1]-centresClean[ii, 1]
                zLocal=dataArrayLocal[:, 2]
                thetaLocal=np.arctan2(yLocal, xLocal)
                radiusLocal=np.sqrt(np.power(xLocal, 2)+np.power(yLocal, 2))
                rpLocal=(radiusLocal-rCollector)/rCollector
                UxLocal=dataArrayLocal[:, 3]
                UyLocal=dataArrayLocal[:, 4]
                UzLocal=dataArrayLocal[:, 5]
                ...
                #if (jj==startIndexTImes):
                print("The testing rp is rpAnalysis=",  rpAnalysis)
                print("These are the rp's:")
                print(rpLocal)
                print("These are the radius's:")
                print(radiusLocal)
                print("These are the x coordinates:")
                print(xLocal)
                print("These are the x coordinates of the full array read:")
                print(dataArray[0::NthetaDiv, 0])
                if diffRp>1E-6:                    
                    print("Radial positions have a problem")
                    print("These are the rp's values calculated and they are very different:")
                    print(rpLocal)
                    print("Therefore, the program is exiting.")
                    sys.exit(7)
                #End if diffRp

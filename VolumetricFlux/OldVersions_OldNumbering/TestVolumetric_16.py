...
####for ii in range(0,NCylinders):
    ...
    if (existingLast<arrayTimeLast):    
        ...
        existingLast=existingTime[-1]
        startIndexTimes=int(np.searchsorted(arrayTimes,existingLast)) #Starting from the next time to the last line
        print("Indexing the start of process")
        print("existingLast: ", existingLast,  " startTime: ",  arrayTimes[startIndexTimes], " previousTime: ",  arrayTimes[startIndexTimes-1])

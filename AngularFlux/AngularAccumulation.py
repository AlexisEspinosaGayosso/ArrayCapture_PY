...
####for ii in range(0,NCylinders):
    ...
    if os.path.isfile(outputName):
        ...
        #End if len(forma)
        print("nLines=", nLines)
        if nLines>2: #Checking for two lines because line 1 is the header
            try:
            ...
            os.remove(outputName) #Deleting the file if we only have 2 lines or less. First line is the theta header
        #End if nLines
    ...
    if (existingLastEnd<arrayTimeLast):
        ...
        existingLastEnd=existingTime[-1, 1]
        existingLastIni=existingTime[-1, 0]
        startIndexTimes=int(np.searchsorted(arrayTimes,existingLastEnd)) #Starting from the End time of the last line
        print("Indexing the start of process")
        print("existingLastIni: ", existingLastIni, " existingLastEnd: ",  existingLastEnd,  " startTime: ",  arrayTimes[startIndexTimes])
        ...
        print("Reading data file: ",  dataNameS)
        ...
        if diffRp>1E-6:                    
            ...
            print("These are the radial positions calculated and they are very different:")
            print(rpLocalS)
        ...
        if (Ntimes == 1):
            ...
            for jj in range(startIndexTimes+1,Ntimes):
                ...
                if ((jj == indexEnd) or (jj == Ntimes-1)):
                    #Writing the new volumetric radial accumulation into this time

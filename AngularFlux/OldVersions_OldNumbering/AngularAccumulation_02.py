...
if len(sys.argv) != NArgNeeded+1:
    ...
    print("The third is the directory caseDir")
    ...
    if not os.path.exists(caseDir):
        ...
        print("You gave: ",  sys.argv[3])
        print("Exiting the script")
        sys.exit(1)
    else:
        volumetricDir=os.path.join(caseDir,"postProcessing","velocityRings")
    #End if not os.path.exisits
#End if len(sys.argv)
...
for ii in range(0, NSX):
    for jj in range(0, NSY):
        for kk in range(0, NCylBase):
            listCylNamesClean.append(cylNamesBase[kk]+"_piece_"+str(ii+1)+"_"+str(jj+1)+"_velocityRings_U.xy") 
...
#OJO recover this after the test
...
for ii in range(0,NCylinders):
    ...
    outputName=os.path.join(outRpDir, (formatRp % rpHere) + "_" + baseName + "_AngularFlux"+".dat") 
    backupName=os.path.join(outRpDir, (formatRp % rpHere) + "_" + baseName + "_AngularFlux" + ".dat"+".bak") 
    print(outputName)
